...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        // TEMPORARY: Allow all requests - no authentication required for testing
        http.authorizeHttpRequests(authorize -> authorize
                .anyRequest().permitAll()
        );

        // Set up OAuth2 resource server for JWT token generation in login endpoint
        http.oauth2ResourceServer(oauth2 -> oauth2.jwt(Customizer.withDefaults()));

        return http.build();
        
        /* ORIGINAL SECURITY CONFIGURATION - Uncomment to restore role-based access control
        ...
        http.authorizeHttpRequests()
                ...
                .requestMatchers(HttpMethod.POST, "/api/public/login").permitAll() // login end-point
                ...
                .requestMatchers(HttpMethod.GET,"/api/books/isbn/search").permitAll() // Public endpoint for ISBN search
                //endBooks
                //readers
