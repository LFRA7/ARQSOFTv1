...
import pt.psoft.g1.psoftg1.bookmanagement.services.ExternalIsbnService;
import pt.psoft.g1.psoftg1.bookmanagement.services.IsbnSearchResult;
...
public class BookController {
    ...
    public ListResponse<BookCountView> getTop5BooksLent() {
        return new ListResponse<>(bookService.findTop5BooksLent());
    }
    ...
    @Operation(summary = "Search for book ISBNs by title using external APIs (Google Books, Open Library)")
    @GetMapping("/isbn/search")
    public ListResponse<IsbnSearchResult> searchIsbnByTitle(
        ...
        List<IsbnSearchResult> results = externalIsbnService.searchIsbnByTitle(title.trim());
        
        if (results.isEmpty()) {
            throw new NotFoundException("No ISBN found for the given title from any external source");
        }
